...
void PrintRSUConfig()
{
    ...
    {
        cout << "Coordinate Phase: ";
        PrintArray<int>(ConfigIS.iCoordinatedPhase, 2);
        cout << "Coordination Cycle: " << ConfigIS.dCoordCycle << endl;
        if (ConfigIS.iCoordinatedPhase[0] > 0)
            cout << "Coordination Split for coordinated phase  " << ConfigIS.iCoordinatedPhase[0] << " is: " << ConfigIS.dCoordinationPhaseSplit[ConfigIS.iCoordinatedPhase[0] - 1] << endl;
        if (ConfigIS.iCoordinatedPhase[1] > 0)
            cout << "Coordination Split for coordinated phase  " << ConfigIS.iCoordinatedPhase[1] << " is: " << ConfigIS.dCoordinationPhaseSplit[ConfigIS.iCoordinatedPhase[1] - 1] << endl;
        cout << "Coordination Offset: " << ConfigIS.dCoordOffset << endl;
        cout << "Transit Weight: " << ConfigIS.dTransitWeight << endl;
        cout << "Truck Weight: " << ConfigIS.dTruckWeight << endl;
        for (int i = 0; i < 8; i++)
        {
            if ((((i + 1) != ConfigIS.iCoordinatedPhase[0]) && ((i + 1) != ConfigIS.iCoordinatedPhase[1])) && (ConfigIS.dCoordinationPhaseSplit[i] > 0))
                cout << " Non-Coordinated Phase " << i + 1 << " split is " << ConfigIS.dCoordinationPhaseSplit[i] << endl;
        }
    }
}
...
void RSUConfig2ConfigFile(char *filename, int *PhaseInfo, int Num)
{
    ...
    int Phase[8]={0};
    double Yellow[8]={0.0}, Red[8]={0.0}, Gmin[8]={0.0}, Gmax[8]={0.0};

    for (int i = 0; i < Num; i++)
    {
        int tempPhase = PhaseInfo[i] - 1;

        if (FindIndexArray(ConfigIS.Phase_Seq_R1, ConfigIS.Ring1No, tempPhase) != -1 || FindIndexArray(ConfigIS.Phase_Seq_R2, ConfigIS.Ring2No, tempPhase % 4) != -1)
...
void ReadInConfig(char *filename, char *filename2)
{
    ...
    ConfigIS.dTransitWeight = dTransitWeight;
    ConfigIS.dTruckWeight = dTruckWeight;
    ConfigIS.dCoordCycle = dCoordCycle;
    ConfigIS.dCoordOffset = dCoordOffset;
    for (int ii = 0; ii < 8; ii++)
        ConfigIS.dCoordinationPhaseSplit[ii] = dCoordPhaseSplit[ii];
