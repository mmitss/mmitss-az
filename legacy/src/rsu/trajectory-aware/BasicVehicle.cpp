...
void BasicVehicle::BSMToVehicle(char* ablob)
{
    ...
    if (ablob != NULL)
    {
		...
		// do the vehicle type (1 bytes) (modified by B.H)
        lTemp=UnpackBlob(offset,ablob,1);
        Vehicletype = (long)lTemp;
	    offset = offset + 1;
        ...
        //!!!!! Debug (B.H) : original code has some problem
        /*      
		byteA = ablob[offset+0];
        byteB = ablob[offset+1];
...
void BasicVehicle::VehicleToBSM(char* ablob)
{
	if (ablob != NULL)
    {
		...
		// do the vehicle type (1 bytes) (modified by B.H)
        lTemp = (long)Vehicletype;
        pByte = (char* ) &lTemp;
		FillupBlob(offset, ablob, pByte,1);
		offset = offset + 1;
