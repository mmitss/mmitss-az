...
int *GeneratePhaseArray(int StartPhase, int *PhaseSeq,int N, int TotalNo, int NoRepeat)  //!!! B.H : GeneratePhaseArray(SP[0],ConfigIS.Phase_Seq_R1,R1No,t1,1)
{
	//GeneratePhaseArray(SP[0],ConfigIS.Phase_Seq_R1,R1No,t1,1);
	if(NoRepeat==0)
	{
...
//!!! B.H : for long term planning => 0,1,2,3,10,11,12,13,20,21,22,23,30,31,32,33,40,41,42,43
int *GeneratePhaseArray_long(int StartPhase, int *PhaseSeq,int N, int TotalNo, int NoRepeat)  //!!! B.H : GeneratePhaseArray(SP[0],ConfigIS.Phase_Seq_R1,R1No,t1,1)
{
	//GeneratePhaseArray(SP[0],ConfigIS.Phase_Seq_R1,R1No,t1,1);
	if(NoRepeat==0)
	{
	...
	else //
	{
		// N is the phase number in a cycle for a ring, so for 8 phase, it is 4, TotalNo is the total number of phases in this sequence.
		int idx=FindIndexArray(PhaseSeq,N, StartPhase);
        int nCycles=9;   //!!! B.H : int nCycles=4; => If there are just two phases in one ring, it needs to be 1,3,11,13,21,23,31,33,41,43,51,53,61,63,71,73,81 for total 16 pahses
        int *phase_array=new int[nCycles*N]; // At most n cycles, each cycle has N elements
        for(int i=0;i<nCycles;i++)
        {
            for(int j=0;j<N;j++)
