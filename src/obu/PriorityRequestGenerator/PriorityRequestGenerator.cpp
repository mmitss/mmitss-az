...
/*
	-If active Map List is not empty obtain directory of active map
*/

std::string PriorityRequestGenerator::getActiveMapFileDirectory(MapManager mapManager)
{

	if (!mapManager.getActiveMapList().empty())
	{
		mapFileDirectory = mapManager.getActiveMapList().front().activeMapFileDirectory;
	}
...
/*
	- If active map List is not empty obtain file name of active map
*/

std::string PriorityRequestGenerator::getActiveMapFileName(MapManager mapManager)
{

	if (!mapManager.getActiveMapList().empty())
	{
		mapFileName = mapManager.getActiveMapList().front().activeMapFileName;
	}
...
void PriorityRequestGenerator::getVehicleInformationFromMAP(MapManager mapManager, BasicVehicle basicVehicle)
{
	...
	if (!mapManager.getActiveMapList().empty())
	{
		...
		fmap = getActiveMapFileDirectory(mapManager);
		intersectionName = getActiveMapFileName(mapManager);
...
void PriorityRequestGenerator::deleteActiveMapfromList(MapManager mapManager)
{
	if(getVehicleIntersectionStatus() == static_cast<int>(MsgEnum::mapLocType::onOutbound))
	{
		mapManager.getActiveMapList().clear();
	}
}
...
bool PriorityRequestGenerator::shouldSendOutRequest(BasicVehicle basicVehicle, MapManager mapManager)
{
	bool bSendRequest = false;
	double vehicleSpeed = basicVehicle.getSpeed_MeterPerSecond();
	std::vector<ActiveRequest>::iterator findVehicleIDOnVector = std::find_if(std::begin(ActiveRequestTable), std::end(ActiveRequestTable),
																			  [&](ActiveRequest const &p) { return p.vehicleID == temporaryVehicleID; });

	if (bgetActiveMap == true) //check if there is active or not
	{
		if (findVehicleIDOnVector == ActiveRequestTable.end()) //If vehicleID is not in the ART, vehicle should send srm
		{
			bSendRequest = true;
		}

		else if (findVehicleIDOnVector != ActiveRequestTable.end() && abs(vehicleSpeed - tempVehicleSpeed) >= VEHICLE_SPEED_DEVIATION_LIMIT) //If vehicleID is in ART and vehicle speed changes by 5m/s, vehicle should send srm. tempVehicleSpeed store the vehicle speed of last send out srm.
		{
			bSendRequest = true;
		}

		else if (findVehicleIDOnVector != ActiveRequestTable.end() && findVehicleIDOnVector->msgCount != msgCount) //If vehicleID is in ART and message count of the last sent out srm and message count in the ART doesn't match, vehicle should send srm
		{
			bSendRequest = true;
		}

		else if (findVehicleIDOnVector != ActiveRequestTable.end() && getVehicleIntersectionStatus() == static_cast<int>(MsgEnum::mapLocType::onOutbound)) //If vehicle is out of the intersection, vehicle should send srm and clear activeMapList
		{
			bSendRequest = true;
			deleteActiveMapfromList(mapManager);
		}
	}
	else
	{
...
/*
	- Define priority status based on vehicle status
		--If vehicle is in inBoundLane priority request type will be priorityRequest
		--If vehicle is in outBoundLane priority request type will be priorityCancellation
		--If vehicle speed changes by 5m/s priority request type will be requestUpdate
*/

int PriorityRequestGenerator::getPriorityRequestType(BasicVehicle basicVehicle)
{
	double vehicleSpeed = basicVehicle.getSpeed_MeterPerSecond();
	std::vector<ActiveRequest>::iterator findVehicleIDOnVector = std::find_if(std::begin(ActiveRequestTable), std::end(ActiveRequestTable),
																			  [&](ActiveRequest const &p) { return p.vehicleID == temporaryVehicleID; });

	if (getVehicleIntersectionStatus() == static_cast<int>(MsgEnum::mapLocType::onInbound)) 
	{
		priorityRequestType = static_cast<int>(MsgEnum::requestType::priorityRequest);
	}

	else if (getVehicleIntersectionStatus() == static_cast<int>(MsgEnum::mapLocType::onOutbound))
	{
		priorityRequestType = static_cast<int>(MsgEnum::requestType::priorityCancellation);
	}

	else if (findVehicleIDOnVector != ActiveRequestTable.end() && abs(vehicleSpeed - tempVehicleSpeed) <= VEHICLE_SPEED_DEVIATION_LIMIT)
	{
		priorityRequestType = static_cast<int>(MsgEnum::requestType::requestUpdate);
	}

	// else if (findVehicleIDOnVector != ActiveRequestTable.end() && vehicleSpeed <= VEHICLEMINSPEED ) //Check whether vehicle will send srm if it is queue
	// {
	// 	priorityRequestType = static_cast<int>(MsgEnum::requestType::requestUpdate);
	// }
...
std::string PriorityRequestGenerator::createSRMJsonObject(BasicVehicle basicVehicle, SignalRequest signalRequest)
{
	...
	signalRequest.setPriorityRequestType(getPriorityRequestType(basicVehicle));
