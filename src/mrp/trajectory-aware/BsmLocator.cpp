...
BsmLocator::BsmLocator(std::string configFilename)
{
...
std::string BsmLocator::readIntersectionMapConfig(std::string configFilename)
{
...
Json::Value BsmLocator::getOnMapFields(Json::Value inputBsmJson)
{

    double latitude = inputBsmJson["BasicVehicle"]["position"]["latitude_DecimalDegree"].asDouble();
    double longitude = inputBsmJson["BasicVehicle"]["position"]["longitude_DecimalDegree"].asDouble();
    double elevation = inputBsmJson["BasicVehicle"]["position"]["elevation_Meter"].asDouble();
    double speed = inputBsmJson["BasicVehicle"]["speed_MeterPerSecond"].asDouble();
    double heading = inputBsmJson["BasicVehicle"]["heading_Degree"].asDouble();
    ...
    bool onMap = plocAwareLib->locateVehicleInMap(connectedVehicle_t_1, vehicleTracking_t_1);

    Json::Value outputBsmJson = inputBsmJson;

    if (onMap == false)
    {
        outputBsmJson["OnmapVehicle"]["onMap"] = false;
        outputBsmJson["OnmapVehicle"]["approachId"] = false;
        outputBsmJson["OnmapVehicle"]["laneId"] = false;
        outputBsmJson["OnmapVehicle"]["signalGroup"] = false;
        outputBsmJson["OnmapVehicle"]["distanceToStopbar"] = false;
        outputBsmJson["OnmapVehicle"]["locationOnMap"] = "outside";
    }
    else
    {
        ...
        outputBsmJson["OnmapVehicle"]["onMap"] = true;
        outputBsmJson["OnmapVehicle"]["approachId"] = vehApproachId;
        outputBsmJson["OnmapVehicle"]["laneId"] = vehLaneId;
        outputBsmJson["OnmapVehicle"]["signalGroup"] = vehSignalGroup;
        outputBsmJson["OnmapVehicle"]["distanceToStopbar"] = vehDistanceToStopBar;
        outputBsmJson["OnmapVehicle"]["locationOnMap"] = locationOnMap;
    }
    return outputBsmJson;
}