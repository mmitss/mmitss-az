...
*/
void PriorityConfig::readPriorityConfig( char * filename)
{
    ...
    if ( dCoordinationWeight>0)
    {
		...
		else if ((iCoordinatedPhase[0]==0 && iCoordinatedPhase[1]>0) || (iCoordinatedPhase[0]>0 && iCoordinatedPhase[1]==0))
		...
		for (int it=0; it<iNumberOfCoordinatedPhase; it++)
		{
			if (iCoordinatedPhase[it]>0)
			{
				...
				cout<< " Coordinated Phase "<<it+1<<" Split :" << dCoordinationPhaseSplit[iCoordinatedPhase[it]-1]<< endl; 
			}	
		}	
		
		if ( ( dCoordinationPhaseSplit[iCoordinatedPhase[0]-1] > 0 ) && ( dCoordinationPhaseSplit[iCoordinatedPhase[0]-1] == dCoordinationPhaseSplit[iCoordinatedPhase[1]-1] ) )
		{
			bSimilarCoordinationSplit=1;
			dSmallerCoordinationSplit=dCoordinationPhaseSplit[iCoordinatedPhase[0]-1]; // in this case , the small and big coordinated phase split time are the same 
			dLargerCoordinationSplit=dSmallerCoordinationSplit;
		}
		
		else if ( dCoordinationPhaseSplit[iCoordinatedPhase[0]-1] != dCoordinationPhaseSplit[iCoordinatedPhase[1]-1] )
		{
			bSimilarCoordinationSplit=0;
			dSmallerCoordinationSplit=min( dCoordinationPhaseSplit[iCoordinatedPhase[0]-1], dCoordinationPhaseSplit[iCoordinatedPhase[1]-1]);
			dLargerCoordinationSplit=max( dCoordinationPhaseSplit[iCoordinatedPhase[0]-1], dCoordinationPhaseSplit[iCoordinatedPhase[1]-1]);
		}
