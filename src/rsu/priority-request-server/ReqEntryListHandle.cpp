...
void UpdateList(LinkedList<ReqEntry> &Req_List, char *RcvMsg, int phaseStatus[8])
{
    ...
    sscanf(RcvMsg,"%s %s %ld %d %f %d %f %lf %d %d %d %d %d %d %d %d %d %d %lf ",msg,RSU_ID,&NewReq.VehID,&NewReq.VehClass,
      ...
      &NewReq.iEndHour,&NewReq.iEndMinute,&NewReq.iEndSecond,&NewReq.iVehState,&NewReq.iMsgCnt,&NewReq.dTimeInCycle);
   // MZP 
   ...
   //---------------Beginning of Handling the Received Requests.----------------//
    ...
    if( ( strcmp(msg,"request") == 0 || strcmp(msg,"coord_request") == 0 ) && NewReq.Phase > 0)  // the vehicle is approaching the intersection or in it is in the queue and requesting a phase or if it is a coordination request
    {
        if(pos<0)  // Request is NOT in the List, problem should be solved by Solver, therefore ReqListUpdateFlag becomes positive
        {
			...
			Req_List.Data().dSetRequestTime, Req_List.Data().iInLane, Req_List.Data().iOutLane, Req_List.Data().iStrHour,Req_List.Data().iStrMinute,Req_List.Data().iStrSecond,
			Req_List.Data().iEndHour,Req_List.Data().iEndMinute,Req_List.Data().iEndSecond,Req_List.Data().iVehState, Req_List.Data().iMsgCnt, NewReq.dUpdateTimeOfETA);
			Req_List.Data().iLeavingCounter=0;
			ReqListUpdateFlag=1;
		    sprintf(temp_log,"*** Add New Request **** { %s }\t \t FLAG  %d at time (%.2f).\n",RcvMsg,ReqListUpdateFlag,dTime);
            outputlog(temp_log);
        }
      ...
      // MZP      outputlog("All the data in the request list is:\n");
          // MZP
			...
			*/
		    if (iRecvReqListDiviser!=iNewReqDiviser) // the recived SRM with identical MsgCnt type is not already in the list, so we should  add/update the new req to the list.  
			{
			// MZP
			 /*
			...
			*/
				if ( (NewReq.VehClass == EV) || (iNumberOfEVinList == 0) ) // We resolve the problem for the new updated request if the new updated request is EV, or there is not EV in the list.
				{
					ReqListUpdateFlag=2;  
				}
    ...
    else if (strcmp(msg,"request_clear")==0) 
    {
		...
		{
			sprintf(temp_log,"A new cancel request is received but the request is not in the list. The request is ignored.\n");
			outputlog(temp_log);
		}
           /*  MZP 12/2/17 
            iRecvReqListDiviser = (int) Req_List.Data().iMsgCnt /10;
...
void PrintList2File(char *Filename,LinkedList<ReqEntry> &ReqList,int IsCombined)
{
    ...
    if (!ReqList.ListEmpty() && pFile!=NULL) 
    {
		if(IsCombined==0)  // output to "requests_combined.txt"
		{
			...
			if(times==1)  //ONLY have one EV will possiblly call split phase. JD 2012.3.27 
			{
				...
				while(!ReqList.EndOfList())
				{
					...
					if(SplitPhase<=0 || ReqList.Data().VehClass!=EV)
					{
						...
						ReqList.Data().iEndHour,ReqList.Data().iEndMinute,ReqList.Data().iEndSecond,ReqList.Data().iVehState, ReqList.Data().iMsgCnt ,ReqList.Data().dTimeInCycle);
					}
					else  // write both the requeste phase (CurPhase) and the split phase (SplitPhase).
					{
						fprintf(pFile,"%s %ld %d %.2f %d %.2f %lf %d %d %d %d %d %d %d %d %d %d %ld %.2lf\n",RSUID.c_str(),ReqList.Data().VehID,ReqList.Data().VehClass,
							ReqList.Data().ETA,CurPhase,ReqList.Data().MinGreen,ReqList.Data().dSetRequestTime,
							 ReqList.Data().iInLane, ReqList.Data().iOutLane, ReqList.Data().iStrHour,ReqList.Data().iStrMinute,ReqList.Data().iStrSecond,
							ReqList.Data().iEndHour,ReqList.Data().iEndMinute,ReqList.Data().iEndSecond,ReqList.Data().iVehState, ReqList.Data().iMsgCnt ,ReqList.Data().dTimeInCycle);
						fprintf(pFile,"%s %ld %d %.2f %d %.2f %lf %d %d %d %d %d %d %d %d %d %d %.2lf\n",RSUID.c_str(),ReqList.Data().VehID,ReqList.Data().VehClass,
			...
			else  // when we have multiple EV at the intersection, there is no split phase for them
			{
				...
				{	
					...
					ReqList.Data().ETA,ReqList.Data().Phase,ReqList.Data().MinGreen,ReqList.Data().dSetRequestTime, 
					ReqList.Data().iInLane, ReqList.Data().iOutLane, ReqList.Data().iStrHour,ReqList.Data().iStrMinute,ReqList.Data().iStrSecond,
					ReqList.Data().iEndHour,ReqList.Data().iEndMinute,ReqList.Data().iEndSecond,ReqList.Data().iVehState, ReqList.Data().iMsgCnt,ReqList.Data().dTimeInCycle);
					ReqList.Next();
				}
		...
		else if(IsCombined==1)  //----- EV will have split phase requests: output "requests.txt": will add split_phase in the sequence of data for each EV request
		{
			TotalReqNum=ReqList.ListSize();
			fprintf(pFile,"Num_req %d %d\n",TotalReqNum,0);
			if (!ReqList.ListEmpty())
			{
				...
				while(!ReqList.EndOfList())
				{
					...
					ReqList.Data().ETA,ReqList.Data().Phase,ReqList.Data().MinGreen,ReqList.Data().dSetRequestTime,ReqList.Data().Split_Phase,
					ReqList.Data().iInLane, ReqList.Data().iOutLane, ReqList.Data().iStrHour,ReqList.Data().iStrMinute,ReqList.Data().iStrSecond,
					ReqList.Data().iEndHour,ReqList.Data().iEndMinute,ReqList.Data().iEndSecond,ReqList.Data().iVehState, ReqList.Data().iMsgCnt,ReqList.Data().dTimeInCycle);	
					ReqList.Next();
				}
...
void deleteThePassedVehicle( LinkedList<ReqEntry> &Req_List )
{
  Req_List.Reset();
    while (!Req_List.EndOfList())
    {
		if (Req_List.Data().iLeavingCounter>=3) // three time steps after the vehicle leaving request is received, the vehicle is deleted from the list.
		{
			Req_List.DeleteAt();
...
void updateETAofRequestsInList( LinkedList<ReqEntry> &Req_List)
{
    ...
    while (!Req_List.EndOfList())
    {
		...
		if ( (dTime - Req_List.Data().dSetRequestTime > iObsoleteTimeOfRemainedReq ) && (Req_List.Data().VehClass != COORDINATION) )// if the received time of the last SRM is (iObsoleteTimeOfRemainingReq second) ago and the SRM has not been updated during this interval, this request is a residual request and should be deleted!
		{
			Req_List.Reset(Req_List.CurrentPosition());
			sprintf(temp_log," ************ Residual request with ID %ld DELETED from the requests list  at time %.2f ************\n",Req_List.Data().VehID,dTime);
			ReqListUpdateFlag=3;  
		    Req_List.DeleteAt();
            outputlog(temp_log);
     // MZP       continue;
		}else
		{
			if ( (Req_List.Data().VehClass==COORDINATION) )
			{
				...
				}else
				{
					if (icoordphase1==Req_List.Data().Phase)
					{
						...
						if (Req_List.Data().ETA>0 && Req_List.Data().MinGreen == 0 )
						{
							if ( (dCurrentTimeInCycle <= max(dcoordphase2split-dcoordphase1split ,0.0 )) ) // if signal is on the coordinated split time , one coordinated phase has ETA and the other has MinGreen 
								Req_List.Data().ETA = max(dcoordphase2split-dcoordphase1split ,0.0 ) - dCurrentTimeInCycle;
							else
								Req_List.Data().ETA = dCoordinationCycle - dCurrentTimeInCycle + max(dcoordphase2split-dcoordphase1split ,0.0 );
						}
					}
					if (icoordphase2==Req_List.Data().Phase)
					{
						...
						if (Req_List.Data().ETA>0 && Req_List.Data().MinGreen == 0 )
						{
							if (dCurrentTimeInCycle <= max(dcoordphase1split-dcoordphase2split ,0.0 ))
								Req_List.Data().ETA = max(dcoordphase1split-dcoordphase2split ,0.0 ) - dCurrentTimeInCycle;
							else
								Req_List.Data().ETA = dCoordinationCycle - dCurrentTimeInCycle + max(dcoordphase1split-dcoordphase2split ,0.0 );
						}
					}		
				}
			...
			}else
			{
				if (Req_List.Data().ETA>0)
				{
					...
					Req_List.Data().ETA=0;	
				// MZP added 10/30/17
				if ((Req_List.Data().iVehState==2) || (Req_List.Data().iVehState==4)) //if the vehicle is leaving intersection
