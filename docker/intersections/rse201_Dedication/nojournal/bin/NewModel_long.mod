...
param cycle, :=0;    # if we have coordination, the cycle length  
param offset,:=0;   # if we have coordination, the offset  
param split, :=0;    # if we have coordination, the split  
...
printf " " > "/nojournal/bin/Results_long.txt";  
printf "%3d  %3d \n ",SP1, SP2 >>"/nojournal/bin/Results_long.txt";  
printf "%5.2f  %5.2f %5.2f  %5.2f \n ",init1, init2,Grn1,Grn2 >>"/nojournal/bin/Results_long.txt";  
for {k in K}   
 { 
     for {p in P2} 
        { 
           printf "%5.2f  ", if(p in P)  then v[p,k] else 0  >>"/nojournal/bin/Results_long.txt";   
        } 
        printf " \n ">>"/nojournal/bin/Results_long.txt";
 } 
...
for {k in K}   
 { 
     for {p in P2} 
        { 
           printf "%5.2f  ", if(p in P)  then g[p,k] else 0  >>"/nojournal/bin/Results_long.txt";   
        } 
        printf " \n ">>"/nojournal/bin/Results_long.txt";
 } 
  
printf "%3d \n ", ReqNo >>"/nojournal/bin/Results_long.txt";  
  
for {p in P,j in J : Rl[p,j]>0}  
 {  
   printf "%d  %5.2f  %5.2f  %5.2f %d \n ", (p+ 10*(theta[p,j])), Rl[p,j],Ru[p,j], d[p,j] , priorityType[j] >>"/nojournal/bin/Results_long.txt";
 } 
  
printf "%5.2f \n ", PriorityDelay >>"/nojournal/bin/Results_long.txt"; 
printf "%5.2f \n ", TotRegVehDel >>"/nojournal/bin/Results_long.txt";  
 printf " \n ">>"/nojournal/bin/Results_long.txt";
end;
