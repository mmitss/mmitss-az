...
set P2  :={1..8};  
set I  :={1..100};  
set T  := {1..20}; 	# at most 10 different types of vehicle may be considered , EV are 1, Transit are 2, Trucks are 3  
...
s.t. GrnMax{p in P : (p!= coordphase1 and p!=coordphase2)}:  g[p,1]<=gmax[p];    
  ...
  s.t. GrnMin2{p in P:(p=coordphase1 and isCurPhCoord1=1)} :g[p,1]>=isItCoordinated*( split-current ); 
  
  s.t. GrnMin3{p in P:(p=coordphase2 and isCurPhCoord2=1)} :g[p,1]>=isItCoordinated*( split-current ); 
  
  s.t. CoordPhasMax1{p in P: p=coordphase1}: isItCoordinated*(1-earlyReturnPhase1)*g[p,1]<=gmax[p]; 
  
  s.t. CoordPhasMax2{p in P: p=coordphase2}: isItCoordinated*(1-earlyReturnPhase2)*g[p,1]<=gmax[p]; 
  
s.t. CoordDelay1{p in P: p=coordphase1}: dc1>=isItCoordinated*((cycle-current+split) - (t[p,2]+v[p,2]) ); 
  
s.t. CoordDelay2{p in P: p=coordphase2}: dc2>=isItCoordinated*((cycle-current+split) - (t[p,2]+v[p,2]) ); 
  
s.t. CoordDelay5{p in P: ((p in P21)and current>gmax[coordphase1] and coordphase1>0)}: dc2>= isItCoordinated*((cycle-current+split) - (t[coordphase2,1]+v[coordphase2,1]));   # I assumed the coordinated phase of ring 2 is in the second brrier 
  
s.t. CoordDelay6{p in P: ((p in P21)and current>gmax[coordphase2] and coordphase1>0)}: dc2>= isItCoordinated*((cycle-current+split) - (t[coordphase2,1]+v[coordphase2,1]));   # I assumed the coordinated phase of ring 2 is in the second brrier 
...
s.t. RegVehDel2{p in P, i in I: active_arr[p,i]>0} : M*miu[p,i] >= i-(t[p,1]+g[p,1]); 
...
s.t. RegVehDel6{p in P, i in I: active_arr[p,i]>0} : t[p,2]+M*(1-miu[p,i])>=tmiu[p,i]; 
  ...
  minimize delay:  TotRegVehDel+  PriorityDelay + coord*(dc1+dc2);     
...
for {p in P,j in J : Rl[p,j]>0}  
 {  
   printf "%d  %5.2f  %5.2f  \n ", (p+ 10*(theta[p,j])), Rl[p,j],Ru[p,j] >>"/nojournal/bin/Results.txt";
 } 
